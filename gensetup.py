...
class setup:
    def __init__(self, proj_path, lib_path, corner):
       # self.path = lib_path
        self.corner = corner
        self.all_file = set()
        self.file_path = set()
    ...
    def getpath(self,key): #--> all_file , --> file_path
        ...
        #for i in self.proj_path:
        for root, dirs, files in os.walk(self.lib_path,followlinks=False):
    ...
    def creat_setup(self,path_key):
        setup_file = self.proj_path + '/setup.tcl'
        print(setup_file)
...
if __name__ == '__main__':
    ...
    haha = setup(proj_path, lib_path, corner)
    haha.run_setup()
